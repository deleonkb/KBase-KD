...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'KDtestContigFilter'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from KDtestContigFilter.KDtestContigFilterImpl import KDtestContigFilter
impl_KDtestContigFilter = KDtestContigFilter(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['KDtestContigFilter.filter_contigs_async'] = ['KDtestContigFilter', 'filter_contigs']
async_check_methods['KDtestContigFilter.filter_contigs_check'] = ['KDtestContigFilter', 'filter_contigs']
sync_methods['KDtestContigFilter.filter_contigs'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'KDtestContigFilter'
        ...
        self.rpc_service.add(impl_KDtestContigFilter.filter_contigs,
                             name='KDtestContigFilter.filter_contigs',
                             types=[dict])
        self.method_authentication['KDtestContigFilter.filter_contigs'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "KDtestContigFilter but no authentication header was passed"
                            raise err
